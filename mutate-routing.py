...
with open('lib/evolution-routing.ts', 'r', encoding='utf-8') as f:
    content = f.read()

new_content = re.sub(
    r'export async function resolveDepartmentForMessage\(input: \{.*?return \{ departmentId: null, departmentName: null.*?\n\}',
    r'''export async function resolveDepartmentForMessage(input: {
  groupJid: string
  subject: string
  text: string | null
...
with open('lib/evolution-routing.ts', 'w', encoding='utf-8') as f:
    f.write(new_content)
