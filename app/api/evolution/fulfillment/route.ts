...
const FULFILLMENT_STAGES = ['CONFIRMED', 'PAYMENT_PENDING', 'ALLOCATED', 'DISPATCH_PENDING', 'DISPATCHED', 'DELIVERED'] as const
...
export async function GET() {
  ...
  const inquiries = await prisma.evolutionDealerInquiry.findMany({
    where: {
      ...
      ...(session.user.role === 'STAFF'
        ? { ticket: { is: { workItems: { some: { departmentId: session.user.routingDepartmentId ?? -1, status: 'ACTIVE' } } } } }
        : {}),
    },
    include: {
  ...
  const totals = {
    ...
    due1: rows.filter((row) => row.order?.countdown.state !== 'OVERDUE' && row.order?.countdown.daysRemaining != null && row.order.countdown.daysRemaining <= 1).length,
    due3: rows.filter((row) => row.order?.countdown.state !== 'OVERDUE' && row.order?.countdown.daysRemaining != null && row.order.countdown.daysRemaining <= 3).length,
    due7: rows.filter((row) => row.order?.countdown.state !== 'OVERDUE' && row.order?.countdown.daysRemaining != null && row.order.countdown.daysRemaining <= 7).length,
