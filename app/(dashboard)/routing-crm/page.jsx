...
import { AlertTriangle, BarChart3, Check, CheckCircle, ChevronLeft, Circle, Link2, Loader2, MessageSquare, Paperclip, RefreshCw, Send, UserCheck, Wifi, WifiOff, X } from 'lucide-react';
    ...
    <div className="flex h-[calc(100vh-7rem)] min-h-[620px] flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
      <div className="flex shrink-0 items-center justify-between gap-4 border-b border-border px-5 py-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {isAdmin && <button type="button" onClick={configureWebhook} className="rounded-lg border border-border px-2.5 py-1.5 text-xs font-semibold text-foreground hover:bg-surface-hover">Register webhook</button>}
      ...
      {metrics && <div className="grid shrink-0 grid-cols-2 gap-px border-b border-border bg-border sm:grid-cols-5">
        ...
        <div className="bg-white px-4 py-2.5"><p className="text-[11px] font-medium text-muted">Median first reply</p><p className="mt-0.5 text-sm font-bold text-foreground">{metrics.response?.medianFirstResponseMinutes == null ? 'â€”' : `${metrics.response.medianFirstResponseMinutes}m`}</p></div>
      ...
      {!connected && isAdmin && <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-accent/20 bg-accent/5 px-5 py-3"><div><p className="text-sm font-semibold text-foreground">Connect the WhatsApp sender</p><p className="text-xs text-muted">Request a QR from Evolution, then scan it on WhatsApp â†’ Linked devices.</p></div><div className="flex items-center gap-2"><button type="button" onClick={connectWhatsApp} disabled={qrLoading} className="inline-flex items-center gap-2 rounded-lg bg-accent px-3 py-2 text-xs font-semibold text-white disabled:opacity-60">{qrLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Link2 className="h-4 w-4" />} Show QR</button><Link href="/settings" className="rounded-lg border border-border px-3 py-2 text-xs font-semibold text-foreground">Configure</Link></div></div>}
...
<h2 className="truncate text-sm font-bold text-foreground">{activeWorkItem.subject}</h2>
                    <p className="text-xs text-muted">{activeWorkItem.departmentName || 'Admin review'} · {(activeWorkItem.routeType === 'inquiry' && activeWorkItem.inquiryId) ? 'Dealer Inquiry' : 'WhatsApp'} · {activeWorkItem.unreadCount > 0 ? <span className="font-medium text-accent">{activeWorkItem.unreadCount} unread</span> : 'Up to date'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {/* Done Custom Button */}
              ...
              {agentRuns.filter((run) => run.status === 'DRAFTED' && run.responseText).slice(0, 1).map((run) => (
                <div key={run.id} className="shrink-0 border-t border-violet-200 bg-violet-50 px-4 py-2.5">
                  <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold text-violet-900">Grounded RAG draft â€” review before sending</p><p className="mt-1 max-w-3xl whitespace-pre-wrap text-xs text-violet-800">{run.responseText}</p><p className="mt-1 text-[10px] text-violet-700">{run.retrievalIds?.length || 0} knowledge reference{run.retrievalIds?.length === 1 ? '' : 's'} Â· {formatTime(run.createdAt)}</p></div><button type="button" onClick={() => setDraft(String(run.responseText || '').replace(/\n\n\[Knowledge refs:.*$/s, ''))} className="shrink-0 rounded-lg border border-violet-300 bg-white px-2.5 py-1.5 text-xs font-semibold text-violet-900 hover:bg-violet-100">Use draft</button></div>
                </div>
              ))}
              <details className="shrink-0 border-t border-border bg-slate-50 px-4 py-2">
                ...
                <div className="mt-2 grid gap-2 lg:grid-cols-3">
                  ...
                  <div className="rounded-lg border border-border bg-white p-2"><p className="mb-1 text-[11px] font-medium text-muted">Convert to dealer order</p>{activeGroup.inquiry?.convertedOrderId ? <><p className="text-xs text-emerald-700">Linked order #{activeGroup.inquiry.convertedOrderId}. Continue items, billing and dispatch in Dealers &amp; Partners.</p><Link href="/dealers" className="mt-2 inline-flex rounded border border-emerald-200 px-2 py-1 text-xs font-semibold text-emerald-800 hover:bg-emerald-50">Open dealer workflow</Link></> : <><p className="mb-2 text-xs text-muted">Requires a linked dealer. Creates an auditable B2B order without inventing material lines.</p><button type="button" onClick={convertToDealerOrder} disabled={convertingOrder || !activeGroup.inquiry?.dealerId} className="rounded bg-emerald-600 px-2 py-1 text-xs font-semibold text-white disabled:opacity-50">{convertingOrder ? 'Creatingâ€¦' : 'Create dealer order'}</button></>}</div>
                </div>
              </details>
              <form onSubmit={sendMessage} className="shrink-0 border-t border-border bg-white p-3">
                ...
                <div className="flex items-end gap-2">
                  ...
                  <textarea value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void sendMessage(event); } }} rows={1} placeholder="Type a reply or attach a fileâ€¦" className="max-h-32 min-h-[42px] flex-1 resize-none rounded-xl border border-border bg-slate-50 px-3 py-2.5 text-sm text-foreground outline-none focus:border-accent" />
